...
namespace POSales
{
    public partial class Category : Form
    {
        ...
        public void LoadCategory()
        {
            ...
            cn.Open();
            cm = new SqlCommand("SELECT * FROM categories ORDER BY name", cn);
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
                i++;
                dgvCategory.Rows.Add(i, dr["id"].ToString(), dr["name"].ToString());
            }
            dr.Close();
            cn.Close();
        }
        ...
        private void dgvCategory_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                ...
                {
                    cn.Open();
                    cm = new SqlCommand("DELETE FROM categories WHERE id LIKE '" + dgvCategory[1, e.RowIndex].Value.ToString() + "'", cn);
                    cm.ExecuteNonQuery();
                    cn.Close();
